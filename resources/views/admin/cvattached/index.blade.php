...
<div class="container mt-2">
	...
	<div class="col-12">
        <div class="card">
			<div class="card-body">
                <h3>@lang('cvattached.managetitle')</h3><br>
                <table id="datatable-buttons" class="table table-striped dt-responsive nowrap w-100">
                    <thead class="mythead">
                        <tr>
                            <th>#</th>
							<th>@lang('cvattached.name')</th>
							<th>@lang('cvattached.roll')</th>
							<th>@lang('cvattached.session')</th>
							<th>@lang('cvattached.department')</th>
							<th>@lang('cvattached.image')</th>
							<th>@lang('cvattached.status')</th>
                    ...
                    <tbody class="tbody" id="showtdata">
                        ...
                        <tr id="tr{{ $showdata->id }}">
                            ...
                            <td>@if($lang == 'en'){{ $showdata->department ?: $showdata->department_name_bn }}@else {{$showdata->department_name_bn ?: $showdata->department}}@endif</td>
                            <td><img src="{{asset('cv_img')}}/{{$showdata->roll}}.{{$showdata->img}}" style="max-height: 50px;"></td>
                            <td>
                                @if($showdata->approve == 1)
                                <a href="{{ url('/Inactivecvattached/'. $showdata->id) }}" class="btn btn-success">Active</a>
                                @else
                                <a href="{{ url('/Activecvattached/'. $showdata->id) }}" class="btn btn-warning">Inactive</a>
                                @endif
                            </td>
                            <td>
                                <div class="btn-group">
                                    <form action="{{ route('cvattached.destroy',$showdata->id) }}" method="post">
                                        @csrf
                                        @method('DELETE')
